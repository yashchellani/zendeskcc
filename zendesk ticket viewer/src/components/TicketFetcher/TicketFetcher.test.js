...
describe('TicketFetcher', () => {
  // Fetch all tickets
  it('HP - ALL TICKETS: should return tickets array when provided correct credentials', async () => {
  ...
  // Fetch ticket by id
  it('HP - SINGLE TICKET: should return a ticket object when provided the correct credentials.', async () => {
    const requester = new TicketFetcher(TOKEN)
  ...
  it('HAPPY PATH: should return the value of the page number passed into the url page query string', () => {
    const requester = new TicketFetcher(TOKEN)
    let pageNumber = 10
    const url = `https://yashc15help.zendesk.com/api/v2/tickets.json?page=${pageNumber}`
