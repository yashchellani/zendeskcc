...
class HttpTicketRequest {
  ...
  async fetchAllTickets() {
    const perPage = 25
    let url = `https://yashc15help.zendesk.com/api/v2/tickets.json?per_page=${perPage}`
    let page = 0
    return async () => {
      if (url != null) {
  ...
  async fetchTicketById(ticketId) {
    let url = `https://yashc15help.zendesk.com/api/v2/tickets/${ticketId}.json`
    let apiResponse = await this.fetchRequest(url)
  ...
  handlesErrors(response) {
    if (!response.ok) {
      ...
      switch (response.status) {
        case 401:
          throw response.statusText + ": Couldn't authenticate!\x1b[0m"
        case 404:
